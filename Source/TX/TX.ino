...
ErrorDetec errorDetec;  // checksum + parity
...
void loop() 
{
    ...
    errorDetec.updateData(lowByte(potValue),highByte(potValue));

    //Serial.println("---------");
    //Serial.println("----LOW-----");
    //Serial.println(lowByte(potValue), BIN);
    //Serial.println(errorDetec.getlowData(), BIN);
   // Serial.println("-----HIGH------");
   // Serial.println(highByte(potValue), BIN);
    //Serial.println(errorDetec.gethighData(), BIN);
    //Serial.println("---------");
    ...
    message[1] = errorDetec.gethighData(); // parte alta de potvalue
    message[2] = errorDetec.getlowData();  // parte baja de potvalue
